...
function CreasTable() {
  ...
  const loadData = async () => {
    try {
      ...
      const createurs = [...new Set(allData.map(item => item.Créateur).filter(Boolean))].sort()
      const types = [...new Set(allData.map(item => item['Type de contenu']).filter(Boolean))].sort()
    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="mb-4 text-sm text-gray-600">
          {data.length} créa{data.length > 1 ? 's' : ''} trouvée{data.length > 1 ? 's' : ''}
        </div>
      )}
